...
impl<Gas: evm::CostType> Gasometer<Gas> {
	...
	pub fn requirements<Shadow: shadow_mem::Shadow>(
		...
		let cost = match instruction {
			...
			instructions::SSTORE => {
				let address = H256::from(&stack.peek(0).0);
				let newval = &stack.peek(1).0;
			...
			instructions::SUICIDE => {
				...
				let address = u256_to_address(&stack.peek(0).0);
				if (
			...
			instructions::MSTORE | instructions::MLOAD => {
				Request::GasMem(default_gas, mem_needed_const(&stack.peek(0).0, 32)?)
			},
			instructions::MSTORE8 => {
				Request::GasMem(default_gas, mem_needed_const(&stack.peek(0).0, 1)?)
			},
			instructions::RETURN | instructions::REVERT => {
				Request::GasMem(default_gas, mem_needed(&stack.peek(0).0, &stack.peek(1).0)?)
			},
			instructions::SHA3 => {
				let words = overflowing!(to_word_size(Gas::from_u256(stack.peek(1).0)?));
				...
				Request::GasMem(gas, mem_needed(&stack.peek(0).0, &stack.peek(1).0)?)
			},
			instructions::CALLDATACOPY | instructions::CODECOPY | instructions::RETURNDATACOPY => {
				Request::GasMemCopy(default_gas, mem_needed(&stack.peek(0).0, &stack.peek(2).0)?, Gas::from_u256(stack.peek(2).0)?)
			},
			instructions::EXTCODECOPY => {
				Request::GasMemCopy(schedule.extcodecopy_base_gas.into(), mem_needed(&stack.peek(1).0, &stack.peek(3).0)?, Gas::from_u256(stack.peek(3).0)?)
			},
			instructions::LOG0 | instructions::LOG1 | instructions::LOG2 | instructions::LOG3 | instructions::LOG4 => {
				...
				let data_gas = overflowing!(Gas::from_u256(stack.peek(1).0)?.overflow_mul(Gas::from(schedule.log_data_gas)));
			...
			instructions::CALL | instructions::CALLCODE => {
				...
				let mem = cmp::max(
					mem_needed(&stack.peek(5).0, &stack.peek(6).0)?,
					mem_needed(&stack.peek(3).0, &stack.peek(4).0)?
				);

				let address = u256_to_address(&stack.peek(1).0);
				let is_value_transfer = !stack.peek(2).0.is_zero();
			...
			instructions::DELEGATECALL | instructions::STATICCALL => {
				...
				let mem = cmp::max(
					mem_needed(&stack.peek(4).0, &stack.peek(5).0)?,
					mem_needed(&stack.peek(2).0, &stack.peek(3).0)?
				);
				let requested = stack.peek(0).0;
			...
			instructions::CREATE => {
				let start = &stack.peek(1).0;
				let len = &stack.peek(2).0;
